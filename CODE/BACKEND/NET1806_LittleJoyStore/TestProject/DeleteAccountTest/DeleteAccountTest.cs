...
namespace TestProject.DeleteAccountTest
{
    public class Tests
    {
        ...
        public static IEnumerable<DeleteAccountEntity> DeleteAccountData()
        {
            var filePath = Path.Combine(TestContext.CurrentContext.TestDirectory, @"..\..\..\..\TestProject\DeleteAccountTest\DeleteAccountData.csv"); //nhớ chỉnh file path
            return DataProvider.GetTestData<DeleteAccountEntity>(filePath);
        }
		...
		[Test, TestCaseSource(nameof(DeleteAccountData))]
        public async Task TestDeleteAccount(DeleteAccountEntity testData)
        {
			...
			var url = $"/api/user?Id={testData.AccountId}";
