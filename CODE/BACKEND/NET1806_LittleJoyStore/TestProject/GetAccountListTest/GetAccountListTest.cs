...
namespace TestProject.GetAccountListTest
{
    public class Tests
    {
        ...
        public static IEnumerable<GetAccountListEntity> GetAccountListData()
        {
            var filePath = Path.Combine(TestContext.CurrentContext.TestDirectory, @"..\..\..\..\TestProject\GetAccountListTest\GetAccountListData.csv"); //nhớ chỉnh file path
            return DataProvider.GetTestData<GetAccountListEntity>(filePath);
        }
		...
		[Test, TestCaseSource(nameof(GetAccountListData))]
        public async Task TestGetAccountList(GetAccountListEntity testData)
        {
			var user = await _mockUserService.Object.GetUserByIdAsync(testData.TesterId);
			...
			var url = $"/api/user?RoleId={testData.RoleId}&Status={testData.Status}";
