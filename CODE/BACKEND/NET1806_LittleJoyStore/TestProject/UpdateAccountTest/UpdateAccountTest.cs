...
namespace TestProject.UpdateAccountTest
{
    public class Tests
    {
        ...
        public static IEnumerable<UpdateAccountEntity> UpdateAccountData()
        {
            var filePath = Path.Combine(TestContext.CurrentContext.TestDirectory, @"..\..\..\..\TestProject\UpdateAccountTest\UpdateAccountData.csv"); //nhớ chỉnh file path
            return DataProvider.GetTestData<UpdateAccountEntity>(filePath);
        }
		...
		[Test, TestCaseSource(nameof(UpdateAccountData))]
        public async Task TestUpdateAccount(UpdateAccountEntity testData)
        {
            ...
            var json = JsonConvert.SerializeObject(new
            {
				...
				status = testData.Status.ToString()=="true"?true:false,
